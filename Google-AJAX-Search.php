...
<?php } ?>
	...
	<script type='text/javascript'> 
		...
		function OnLoad()
		{
			...
			webSearch.setSiteRestriction("<?php echo get_option('siteurl'); ?>");
			webSearch.setUserDefinedLabel("Web");
			webSearch.setUserDefinedClassSuffix("webSearch");
				<?php if( get_option('googleajaxsearch_googleAjaxBSOption') == 'true') { ?>
			var blogSearch = new GblogSearch(); 
			blogSearch.setSiteRestriction("<?php echo get_option('siteurl'); ?>");
	...
	<!-- Google AJAX Search Plugin for WordPress (end) -->
...
function googleajaxsearch_optionsSubpanel() {
	if($_POST['action'] == "save") {
		...
		<td><br/>
		<input type="text" name="open" size="7" value="<?php echo get_option('googleajaxsearch_googleAjaxopenoption'); ?>"><br/>Type 'OPEN' or 'CLOSED' (without quotes)
		</td>
		</tr>		
	...
	function widget_gajaxsearch($args) {
		...
		?>
				<div id="searchcontrol">
					<form method="get" action="http://blogsearch.google.com/blogsearch"><div>
                                        <input name="as_q" size="16" maxlength="255" value="" type="text" />
					<input name="sa" value="Search" type="submit" />
					<input name="bl_url" value="<?php echo get_option('siteurl'); ?>" type="hidden" />
					</div>
                                        </form>
				</div>
	...
	?>
			<div id="searchcontrol">
				...
				<div>
                                <input name="as_q" size="16" maxlength="255" value="" type="text" />
				<input name="sa" value="Search" type="submit" />
				<input name="bl_url" value="<?php echo get_option('siteurl'); ?>" type="hidden" />                </div>
				</form>
			</div>
	<?php }  // end gajaxsearch()
